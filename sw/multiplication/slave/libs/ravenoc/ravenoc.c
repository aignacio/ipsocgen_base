...
void vRaveNoCInitNoCLUT (void) {
  ...
  for (size_t i=0,x=0,y=0; i<slaveNOC_TOTAL_TILES; i++) {
    ucgTileLUTx[i] = x;
    ucgTileLUTy[i] = y;
...
void vRaveNoCPrintNoCSettings (void) {
  ...
  dbg("\n\rNoC Size: %dx%d", slaveNOC_SIZE_X, slaveNOC_SIZE_Y);
...
void vRaveNoCSendNoCMsg (uint8_t ucTileNumber, uint8_t ucPktSize, uint32_t ulData) {
  if (ucTileNumber > (slaveNOC_TOTAL_TILES)-1) {
    dbg("\n\r[RaveNoC-Error] Tile number out of range, please choose between 0 to %d!", slaveNOC_TOTAL_TILES-1);
  }
