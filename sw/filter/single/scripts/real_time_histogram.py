...
class App(QWidget):
    ...
    lineGray, = ax.plot(np.arange(bins), np.zeros((bins,1)), c='k', lw=3)
    ...
    def update_hist(self, cv_img):
        ...
        histogram = cv2.calcHist([frame], [0], None, [self.bins], [0, self.bins])
        histogram = histogram/histogram.max()
        #histogram = histogram/histogram.sum() # Normalized
