...
sock.bind(("", UDP_PORT))
numPixels = np.prod(frame.shape[:2])//SEG
# cap = cv2.VideoCapture(0)
...
while (1):
    ...
    for r in range(numPixels):
        msg = frame_array[r*SEG:(r*SEG+SEG)].tobytes()
    ...
    #data = IrisMPSoCPkt(0,IrisMPSoCEnc.CMD_TEST, (IMG_WIDTH-1,IMG_HEIGHT-1)).get_header()
    #sock.sendto(data, (UDP_IP, UDP_PORT))
    #rx_eth = sock.recvfrom(4)
    #message = rx_eth[0]
    #type(message)
    #print("Tick counter: %d"%int.from_bytes(message, byteorder='little'))
    #time.sleep(0.001)
